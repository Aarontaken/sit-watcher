...
前往 [Releases](https://github.com/Aarontaken/sit-watcher/releases) 下载最新的 `SitWatcher.dmg`，打开后将 `SitWatcher.app` 拖入「应用程序」文件夹。

首次打开时，macOS Gatekeeper 会阻止运行未公证的应用。请选择以下任一方式：

**方式一（推荐）：终端命令移除隔离标记**
```bash
xattr -d com.apple.quarantine /Applications/SitWatcher.app
```

**方式二：系统设置中允许**
1. 双击打开应用，看到警告后点击「完成」
2. 前往「系统设置 → 隐私与安全性」
3. 在页面底部找到 "SitWatcher" 并点击「仍要打开」
