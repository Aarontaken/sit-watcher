...
每次执行都会从 **`master`** 拉取最新的 `install.sh`，再用 GitHub **Releases 的最新 tag** 匹配 **`gh-pages`** 上的 `SitWatcher-{版本}.zip`（版本号不含前缀 `v`），解压后安装到 `/Applications` 并启动。**无需手动指定版本。**

```bash
curl -fsSL https://raw.githubusercontent.com/Aarontaken/sit-watcher/master/install.sh | bash
```

若默认分支不是 `master`，把 URL 里的路径改成实际分支名即可。极少数情况下需要锁定某一 tag 时再使用：`… | bash -s -- v1.0.3`。
